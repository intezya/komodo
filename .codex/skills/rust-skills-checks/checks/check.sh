...
DEFAULT_ROOT="$(cd "$CHECKS/../../rust-skills" && pwd)"
ROOT="${RUST_SKILLS_ROOT:-$DEFAULT_ROOT}"
TARGET="x86_64-unknown-linux-gnu"

echo "==> structure, links, and index parity"
RUST_SKILLS_ROOT="$ROOT" python3 "$CHECKS/validate.py"
RUST_SKILLS_ROOT="$ROOT" python3 "$CHECKS/gen_index.py" --check

echo "==> generating example files from rules"
cd "$CHECKS"
RUST_SKILLS_ROOT="$ROOT" python3 gen.py

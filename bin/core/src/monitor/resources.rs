...
use crate::{
  helpers::query::{
    ...
    get_stack_state_from_containers,
  },
  stack::services::extract_services_from_stack,
  state::{
...
pub async fn update_server_stack_cache(
  ...
  for stack in stacks {
    let services = extract_services_from_stack(&stack);
    let project_name = stack.project_name(false);
    let mut services_with_containers = services
      .iter()
      .map(
        |service_meta @ StackServiceNames {
           service_name,
           image,
           desired_replicas,
           ..
          ...
          let service_containers = containers
            ...
            .cloned()
            .collect::<Vec<_>>();
          let container = service_containers.first().cloned();
